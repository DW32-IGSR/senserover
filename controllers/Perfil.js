    ...
    if(sess.usuario==""||sess.usuario==undefined) {
        ...
        if (errors) {
          return res.redirect('/');
            ...
            Usuario.findOneAndUpdate({ _id: sess.id_usuario }, { nombre: form_nombre, apellidos : form_apellidos, dni: form_dni, direccion : form_direccion, codigo_postal : form_cp, email :  form_email}, function(err, user) {
                if (err) {
                    console.error(err)
                } else {
